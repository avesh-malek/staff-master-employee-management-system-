...
const Navbar = ({ setSidebarOpen }) => {
  ...
  return (
    <nav
      className="bg-slate-900 border-b border-slate-800 px-4 py-2 
flex items-center justify-between 
fixed top-0 left-0 w-full z-50 h-[50px]"
    >
      ...
      <div className="flex items-center gap-4">
        <span className="text-slate-300 text-xs sm:text-sm truncate max-w-[120px] sm:max-w-none">
          {user?.name} ({user?.role})
        </span>
        <button
          onClick={() =>
